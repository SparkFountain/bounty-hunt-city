...
skin\Fnt_Gadgets = GUI_LoadFont("Tahoma", 13, 0)	;GUI_GetParam("Fnt_Gadgets$")
...
Function GUI_CreateButton(owner, x = 0, y = 0, w = 75, h = 21, txt$ = "Button", icon$ = "", enabled = True, show = True)
...
For gad.GUI_Button = Each GUI_Button
	If gad\owner = owner Then
		...
		If gad\enabled Then
			...
			If gad\op Then f = 4
		EndIf
		If gad\show Then DrawImage gad\img[f], gui\gadox, gui\gadoy
		If gad\icon Then 
			If gad\show Then DrawImage gad\icon, gui\gadox + gad\w / 2 - ImageWidth(gad\icon) / 2, gui\gadoy + gad\h / 2 - ImageHeight(gad\icon) / 2
		EndIf
		x = gui\gadox + gad\w / 2
		y = gui\gadoy + gad\h / 2
		If gad\enabled Then
	...
	Case "btn"
		...
		Select msg$
			...
			Case "seticon"
				...
				Else
					btn\icon = GUI_LoadIcon(p1$,p2$,p3$)
				EndIf
			Case "getenabled"
				Return btn\enabled
			Case "setenabled"
...
Function GUI_LoadFont(fnt$="Courier New",height=14,bold=0,italic=0,underline=0)
	Return LoadFont(fnt$,height,bold,italic,underline)	;GUI_Parse(fnt$, 0), GUI_Parse(fnt$, 1), GUI_Parse(fnt$, 2), GUI_Parse(fnt$, 3), GUI_Parse(fnt$, 4)
End Function
...
Function GUI_LoadIcon(path$, width=-1, height=-1)
If FileType(path$) = 1 Then
	img = LoadImage(path$)
	If(width<>-1 And height<>-1) Then ResizeImage(img,width,height)
;	If(width=-1) Then
;		w = ImageWidth(img)
;	Else
;		w = width
...
Function GUI_MsgBox(title$ = "Message", msg$ = "Message", buttons = 1, offsetX = -1)
FlushKeys()
skin.GUI_Skin = gui\skin
SetFont skin\fnt_gadgets
w = 275
...
For i = 0 To cnt_items
	...
	If nw > w Then w = nw
Next
win = GUI_CreateWindow(offsetX, -1, w, h, title$, "", True, False, False)
For i = 0 To cnt_items
